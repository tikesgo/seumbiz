...
const IMAGE_BASE_URL = "http://localhost:4173";
...
const liveProgressSamples = [
  { productName: "문화상품권", amount: 50000, customerName: "김**", status: "대기" },
  { productName: "구글기프트카드", amount: 30000, customerName: "박**", status: "접수완료" },
  { productName: "신세계상품권", amount: 100000, customerName: "이**", status: "승인" },
  { productName: "롯데모바일", amount: 50000, customerName: "최**", status: "대기" },
  { productName: "네이버페이", amount: 30000, customerName: "한**", status: "취소" },
];
...
const renderGiftPills = () => {
  ...
  giftProducts.forEach((product) => {
    ...
    const icon = createElement("span", "gift-icon", "");
    renderProductLogo(icon, product);
...
const parseRateValue = (rate) => {
  const number = Number(String(rate || "").replace(/[^\d.-]/g, ""));
...
const getProductImageUrl = (product) => resolveImageUrl(product?.imageUrl || product?.image_url || "");

const renderProductLogo = (container, product, className = "gift-icon") => {
  const badge = product?.badge || product?.name?.slice(0, 1) || "상";
  const imageUrl = getProductImageUrl(product);

  container.className = className;
  container.textContent = badge;
  container.classList.remove("has-image");
  container.style.setProperty("--icon-color", product?.color || "#2478ff");

  if (!imageUrl) return;

  const image = document.createElement("img");
  image.src = imageUrl;
  image.alt = `${product?.name || product?.giftType || "상품권"} 로고`;
...
const normalizeGiftcardType = (row) => ({
  ...
  imageUrl: resolveImageUrl(row.image_url || row.imageUrl || ""),
...
const renderNotices = (notices) => {
  if (!noticeSummaryList) return;
  noticeSummaryList.replaceChildren();

  if (!notices?.length) {
    const empty = createElement("article", "", "");
    empty.append(createElement("strong", "", "현재 등록된 공지가 없습니다."), createElement("span", "", ""));
    noticeSummaryList.append(empty);
    return;
  }

  const fragment = document.createDocumentFragment();
  notices.slice(0, 5).forEach((notice, index) => {
    const article = createElement("article", "notice-summary-item", "");
    const button = createElement("button", "notice-summary-button", "");
    const left = createElement("span", "notice-summary-left", "");
    const number = createElement("span", "notice-summary-number", `${index + 1}.`);
    const titleText = createElement("strong", "notice-summary-title", notice.title || notice.category || "공지사항");
    const meta = createElement("span", "notice-summary-meta", "");
    const arrow = createElement("span", "notice-summary-arrow", ">");
    const title = notice.title || notice.category || "공지사항";

    left.append(number, titleText);
    button.type = "button";
    button.setAttribute("aria-label", `${title} 공지 자세히 보기`);
    meta.append(arrow);
    button.append(left, meta);
    button.addEventListener("click", () => openNoticeDetail(notice));
    article.append(button);
    fragment.append(article);
  });
  noticeSummaryList.append(fragment);
};
...
const formatNoticeDetailDate = (value) => {
  if (!value) return "-";
  const date = new Date(value);
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
...
const closeNoticeDetail = () => {
  if (!noticeDetailModal) return;
  noticeDetailModal.remove();
  noticeDetailModal = null;
...
const openNoticeDetail = (notice) => {
  ...
  const modal = createElement("article", "bank-modal notice-detail-modal", "");
  const closeButton = createElement("button", "modal-close", "×");
  const badgeRow = createElement("div", "notice-detail-badge-row", "");
  const title = createElement("h2", "", notice.title || "공지사항");
  const date = createElement("p", "notice-detail-date", `작성일 ${formatNoticeDetailDate(notice.created_at || notice.date)}`);
  const content = createElement("div", "notice-detail-content", "");
  const closeAction = createElement("button", "notice-detail-close-button", "닫기");

  closeButton.type = "button";
  closeButton.setAttribute("aria-label", "공지사항 닫기");
  closeButton.addEventListener("click", closeNoticeDetail);
  closeAction.type = "button";
  closeAction.addEventListener("click", closeNoticeDetail);
  if (notice.is_important) {
    badgeRow.append(createElement("span", "notice-important-badge", "중요공지"));
  }
  content.textContent = notice.content || notice.description || notice.summary || "";
  modal.append(closeButton, badgeRow, title, date, content, closeAction);
...
const loadNotices = async () => {
  ...
  renderNotices(data.filter((notice) => !notice.scheduled_at || new Date(notice.scheduled_at) <= new Date()));
};
...
const createLiveProgressItem = (item) => {
  ...
  const productCell = createElement("div", "main-live-product-cell", "");
  const productName = item.productName || item.giftType || "상품권";
  const product = giftProducts.find((gift) => gift.name === productName) || { ...item, name: productName };
  const icon = createElement("span", "main-live-icon", "");
  renderProductLogo(icon, product, "main-live-icon");
  const productText = createElement("strong", "main-live-product", productName);
  const amountText = createElement("em", "main-live-amount", formatWon(Number(item.amount || 0)));
  const customerText = createElement("b", "main-live-customer", item.customerName || item.customer || "***");
  const status = item.status || "접수완료";
  const statusBadge = createElement("span", `status-badge status-${status}`, status);

  productCell.append(icon, productText);
  article.append(productCell, amountText, customerText, statusBadge);
  return article;
};

const getOrderProductName = (order) => {
  const items = Array.isArray(order.items) ? order.items : [];
  return items[0]?.productName || items[0]?.product_name || items[0]?.name || order.product_name || "상품권";
};

const getOrderAmount = (order) => {
  const items = Array.isArray(order.items) ? order.items : [];
  const itemTotal = items.reduce((sum, item) => sum + Number(item.amount || item.price || 0), 0);
  return itemTotal || Number(order.total_amount || order.totalAmount || 0);
};

const normalizeLiveProgressOrder = (order) => ({
  productName: getOrderProductName(order),
  amount: getOrderAmount(order),
  customerName: maskCustomerName(order.account_holder || order.accountHolder || order.name || ""),
  status: order.status || "접수완료",
});

const loadLiveProgressItems = async () => {
  const { data, error } = await supabase
    .from("orders")
    .select("id, account_holder, name, total_amount, status, items, created_at")
    .order("created_at", { ascending: false })
    .limit(12);

  if (error || !data?.length) {
    return liveProgressSamples;
  }

  return data.map(normalizeLiveProgressOrder);
};

const renderLiveProgressTrack = (items) => {
  const track = createElement("div", "main-live-track", "");
  const firstSet = createElement("div", "main-live-set", "");
  const secondSet = createElement("div", "main-live-set", "");

  items.forEach((item) => firstSet.append(createLiveProgressItem(item)));
  items.forEach((item) => secondSet.append(createLiveProgressItem(item)));
...
const initMainLiveRolling = async () => {
  if (!mainLiveList) return;
  renderLiveProgressTrack(await loadLiveProgressItems());
};
...
const renderDetail = (product) => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    registeredGifts.push({
      ...
      rate: parseRateValue(product.rate),
      pins,
      pinKey,
  ...
  try {
    await saveReceipt({
      ...
      items: registeredGifts.map((item) => ({
        ...
        amount: item.amount,
        rate: item.rate,
        payoutAmount: Math.round((item.amount * item.rate) / 100),
