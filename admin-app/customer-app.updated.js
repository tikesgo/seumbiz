...
const normalizeGiftcardType = (row) => ({
  ...
  rate: formatRate(row.rate || row.buy_rate || row.purchase_rate),
  description: row.description || "핀번호 4칸과 상품권 금액을 입력한 뒤 등록해주세요.",
  help: Array.isArray(row.help)
    ? row.help
...
const loadGiftcardTypes = async () => {
  ...
  console.log("giftcard_types data:", data);
  console.log("giftcard_types error:", error);

  if (error) {
    console.error("giftcard_types 조회 오류:", error);
    giftProducts = fallbackGiftProducts;
    return;
  }

  if (!data?.length) {
    console.warn("활성화된 giftcard_types 데이터가 없습니다.");
    giftProducts = fallbackGiftProducts;
    return;
  }
...
const saveReceipt = async (receipt) => {
  const response = await fetch(`${ADMIN_API_BASE}/api/orders`, {
    method: "POST",
...
const renderDetail = (product) => {
  ...
  selectedAmount = amountOptions[0].value;
